...
-- 切换任务
function Main.changeTask()
    toast("changeTask", 3)
    mSleep(3000)
    -- 正在执行的任务序号+1
    taskRecord.currentTaskIndex = taskRecord.currentTaskIndex + 1
    -- 一个号的任务做完了
    if taskRecord.currentTaskIndex > #taskRecord.taskList then
        toast("next accont", 3)
        -- thread.stop(mainThread.appThread)
        -- thread.stop(mainThread.stuckThread)

        -- 登录下一个账号
        UISetting.accountIndex = UISetting.accountIndex + 1
        mainStatus.isNextAccount = 1
        return
    end

    if taskRecord.currentTaskIndex <= 4 then
        -- 取出任务值
        taskNum = taskRecord.taskList[taskRecord.currentTaskIndex]
    else
        taskNum = taskRecord.currentTaskIndex
    end
...
function Main.login()
    if UISetting.accountIndex > #UISetting.accountList then
        Common.record("全部账号执行成功")
        return
    end
    UISetting.currentAccount = UISetting.accountList[UISetting.accountIndex]
