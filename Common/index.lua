...
local function changanLocation()
    local offset = '59|5|0xc24e10,64|16|0xb6c3bb,3|18|0x96a29b,22|10|0xb62a09,32|10|0xbb290a'
...
function Common.wordCancle(rate, second)
    ...
    local offset = '4|1|0xefc263,4|12|0xecbc5f,14|5|0xedc162,25|0|0x4a321f,48|9|0x48301e,35|7|0xe5b95e,35|16|0xebbb5e'
...
function Common.move(breakFunction, moveFunction, pointFunction, resetFunction, pageFunction)
...
function Common.userDialog(rate, second, type)
    ...
    if type == 1 then
        ...
        return findColorsUntil(0x222323, offset2, 90, 33, 582, 329, 619, {orient = 2}, 500, 1)
    else
        -- 剧情对话框
        local offset1 = '11|0|0xdec697,46|0|0xdec697,90|0|0xdec698,184|15|0xe4d399,-25|18|0xe8dc9b'
        return findColorsUntil(0xdec698, offset1, 90, 0, 447, 1133, 635, {orient = 2}, 500, 1)
    end
end
...
function Common.teamPage()
    ...
    return findColorsUntil(0xcd0000, offset, 90, 93, 17, 1015, 136, {orient = 2}, 500, 1)
end
...
function Common.easyGroupPage()
    ...
    return findColorsUntil(0xce0000, offset, 90, 72, 6, 1055, 105, {orient = 2}, 500, 1)
end
