        ...
        pages = {
            ...
            },
            {
                ...
                },
                {
                    ['type'] = 'CheckBoxGroup',
                    ['id'] = 'sell_shanghui',
                    ['list'] = '精铁,鬼谷子,黑宝石,太阳石,神秘石,舍利子,翡翠石,红纹石,月亮石,光芒石,昆仑玉,月华露,锻造册,制造书',
                    ['select'] = '0@1@4@7@10@11@12@13',
                    ['countperline'] = 3
                },
                {
                    ['type'] = 'Label',
...
local function analysisAccount(accountStr)
    local accountInfoList = strSplit(accountStr, '\n')

    local accountObjList = {}

    for index = 1, #accountInfoList do
        local accountInfo = accountInfoList[index]

        local accountDetail = strSplit(accountInfo, '-')

        local account = accountDetail[1]
        local passwd = accountDetail[2]
        local server = accountDetail[3]
        local roleNum = accountDetail[4]
...
local function analysisLianxiaohao(lianxiaohaoStr)
    local lianxiaohaoInfoList = strSplit(lianxiaohaoStr, '\n')

    local lianxiaohaoObjList = {}

    for index = 1, #lianxiaohaoInfoList do
        local lianxiaohaoInfo = lianxiaohaoInfoList[index]

        local lianxiaohaoDetail = strSplit(lianxiaohaoInfo, '-')
        local renwu = lianxiaohaoDetail[1]
        local menpai = lianxiaohaoDetail[2]

        local lianxiaohaoObj = {['renwu'] = renwu, ['menpai'] = menpai}
...
local function bag_analysis(uiValues)
