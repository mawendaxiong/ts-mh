...
local function masterControl()
    while true do
        ...
        if mainStatus.isNextAccount ~= -1 then -- 登录下一个账号
            ...
            mainThread.loginThread = thread.createSubThread(Main.login, threadCallbackTable)
        end
...
#################thread################
    ...
    -- 登录线程重启游戏,并输入账号登录游戏
    thread.create(Main.login)

    thread.create(masterControl,mainThreadCallbackTable)
